...
if( strcmp(diag_cov,"yes") == 0){
for(j=0;j<Ncov;j++)
{
for(i=0;i<Ncov;i++)
{
if(j==i){}//nothing
else{Variance[j+i*Ncov]=0;}//set to be 0
}}
}
...
//   if(j>1){Variance[j+i*Ncov]=;}//do nothing, auto k-part not modified
   ...
   if(Variance[j+Ncov*i]-Variance[j+Ncov*i]!=0){printf("Error with Variance cov (i,j)=(%d,%d), cov[%d]=%lf. Exiting now...\n",i,j,j+Ncov*i, Variance[j+Ncov*i]);exit(0);}
  ...
  for(i=0;i<output;i++)//filas compression matrix
  {
     for(j=0;j<output;j++)//columnas compression matrix
     {
         ...
         for(ii=0;ii<Ncov;ii++)//filas original matrix
         {
             ...
             if(Variance_comp[j+output*i]-Variance_comp[j+output*i]!=0){printf("Error with Variance cov (i,j)=(%d,%d), cov[%d]=%lf. Exiting now...\n",i,j,j+output*i, Variance_comp[j+output*i]);exit(0);}
...
//           cov[j+Ncov*i]=1./((1.-(Ncov+1.)/(Ncounter*1.-1.))*gsl_matrix_get (inverse, i, j));
             ...
             if(i==j){if(cov[j+Ncov*i]-cov[j+Ncov*i]!=0){printf("Error with inverse cov (i,j)=(%d,%d), cov[%d]=%lf. Exiting now...\n",i,j,j+Ncov*i, cov[j+Ncov*i]);exit(0);}}//only diagonals
